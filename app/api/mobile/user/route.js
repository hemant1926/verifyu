  ...
  try {
    ...
    const userDetails = await prisma.user.findUnique({
      where: {
        ...
        // id: "a9fea912-4b1b-4a1c-8bf1-269a088b4811",
        status: 1,
      },
      include: {
...
export async function PUT(req) {
  try {
    ...
    // for(let child of JSON.parse(formData.get("childDetails"))) {
    //   if (child.photo != null) {
    //     if(formData.get("childphoto"+j)){
    //     let photo = formData.get("childphoto"+j);
    //     // Find the existing photo path to delete it
